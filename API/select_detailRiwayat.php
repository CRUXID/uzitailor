<?php 
    ...
    $result = mysqli_query( $koneksi,"SELECT kode_transaksi,
        DATE(transaksi.waktu) AS waktu,total,dibayar,tgl_jadi FROM transaksi WHERE status = 4  AND kode_transaksi = '$kode'AND id_pembeli = $id ");
    ...
    detail_transaksi.qty,detail_transaksi.sub_total, transaksi.id_pembeli,detail_transaksi.catatan FROM detail_transaksi JOIN master_barang 
         ...
         WHERE detail_transaksi.kode_transaksi = '$kode' AND transaksi.id_pembeli = $id AND transaksi.status = 4  "); 
