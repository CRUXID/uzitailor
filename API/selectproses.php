<?php 
    ...
    $result = mysqli_query( $koneksi,"SELECT kode_transaksi,
        DATE(transaksi.waktu) AS waktu,total FROM transaksi WHERE status = 2  AND id_pembeli = $id ");
    ...
    detail_transaksi.qty,detail_transaksi.sub_total, transaksi.id_pembeli FROM detail_transaksi JOIN master_barang 
         ...
         WHERE detail_transaksi.kode_transaksi = '$row3[kode_transaksi]' AND transaksi.id_pembeli = $id AND transaksi.status = 2  "); 
